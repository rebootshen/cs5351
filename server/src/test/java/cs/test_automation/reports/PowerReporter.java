...
//import cs.test_automation.reports.TspDataFile;
 ...
 */
public class PowerReporter implements IReporter {
	private static final Logger L = Logger.getLogger(PowerReporter.class);
	...
	private TspDataFile tspFile = new TspDataFile();
		...
		if (tests.getAllResults().size() > 0) {
			...
			for (ITestNGMethod method : getMethodSet(tests, suite)) {
				...
				if (!className.equalsIgnoreCase(lastClassName)) {
					if (mq > 0) {
						...
						if(!lastClassName.equals("")){
							testClasses.add(lastClassName+" "+mq + " " + totalTime+"\n");
						}
					}
				...
				buff.append("<td><a href=\"#m" + methodId + "\">" + qualifiedName(method) + " "
						...
						+ convertDate(start) + "</td>" + "<td class=\"numi\">" + (end - start) +" [" + totalTime + "]</td>" + "</tr>");
			}
			if (mq > 0) {
				cq += 1;
				m_out.print("<tr class=\"" + style + (cq % 2 == 0 ? "even" : "odd") + "\">" + "<td");
	...
	private void tableStart(String cssclass, String id) {
		m_out.println("<table border=\"1\" cellspacing=\"0\" cellpadding=\"0\""
